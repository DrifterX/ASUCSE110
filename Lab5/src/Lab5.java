// AUTHOR: Nic Holmes
// FILENAME: Lab5.java
// SPECIFICATION: Making diamonds and pyramids
// FOR: CSE 110
...
public class Lab5 {
    public static void main(String[] args) {
        ...
        do {
            ...
            case 'p':
                ...
                numSymbols = checkIfOdd(numSymbols);
                System.out.println();
    ...
    private static String buildDiamond(
        ...
        int totalSymbOneRow = numSymbols;
        for(int numBlanks = 1; numBlanks <= numSymbols; numBlanks +=2) {
        	int numStars = (totalSymbOneRow - numBlanks) / 2;
        	String row = buildRow(sideSymb, numStars, midSymb, numBlanks);
        	result += row + "\n";
        }
        
        for(int numBlanks = (numSymbols-2); numBlanks >= 1; numBlanks -=2) {
        	int numStars = (totalSymbOneRow - numBlanks) / 2;
        	String row = buildRow(sideSymb, numStars, midSymb, numBlanks);
        	result += row + "\n";
        }
    ...
    private static int checkIfOdd(int numToCheck) {
    	if(numToCheck % 2 != 0) {
    		return numToCheck;
    	}
    	else {
    		System.out.println("The input is not an odd number. Use the closest odd number below " + numToCheck);
    		return numToCheck - 1;
    	}
    }
